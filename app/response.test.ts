...
describe("Response", () => {
  ...
  it("Can gzip the response body", async () => {
    ...
    const body = Buffer.copyBytesFrom(responseData, responseData.byteLength - contentLength, contentLength);
  ...
  it("Can set the response version", async () => {
    ...
    expect(await toString(response)).toBe("HTTP/1.0 200 OK\r\ncontent-length: 0\r\n\r\n");
  });
  ...
  it("Uses Unknown for a message with an unknown status code", async () => {
    ...
    expect(await toString(response)).toBe("HTTP/1.1 999 Unknown\r\ncontent-length: 0\r\n\r\n");
  });

  it("Sets the content encoding header when setting compression", async () => {
    const response = new Response();
    response.setCompression("gzip");
  ...
  it("Clears the content encoding header when setting compression to uncompressed", async () => {
    const response = new Response();
